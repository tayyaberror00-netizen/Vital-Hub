...
// Prefix all asset paths with the Django static URL injected via meta tag
(function () {
    const base = document.querySelector('meta[name="static-prefix"]')?.content ?? '';
    if (!base) return;
    products.forEach(p => {
        p.img    = base + p.img;
        p.images = (p.images || []).map(i => base + i);
        if (p.modelFile) p.modelFile = base + p.modelFile;
        if (p.videoFile) p.videoFile = base + p.videoFile;
    });
})();
